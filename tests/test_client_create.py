...
class ClientCreator:
    def __init__(self, host='localhost', port=55555, username='Player1'):
            ...
            try:
                ...
                while '\n' in buffer:
                    ...
                    if message_str:
                        ...
                        print(f"\n[SERVER] {message}")
                        # Store the room code when the room is created
                        if message.get("type") == "room_created":
                            self.room_code = message.get("payload", {}).get("room_code")
                            print(f"\n[INFO] Room created with code: {self.room_code}. Waiting for another player...")
